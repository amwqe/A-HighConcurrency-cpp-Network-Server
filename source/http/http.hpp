...
#define DEFAULT_TIMEOUT 10  //默认超时时为10s
...
class Util {
    public:
        ...
        static bool WriteFile(const std::string &filename, const std::string &buf) {
            std::ofstream ofs(filename, std::ios::binary | std::ios::trunc);  // std::ios::trunc:截断
        ...
        //RFC3986文档规定，编码格式 %HH ，HH是一个2位16进制数，范围是00~FF
...
class HttpRequest {
    ...
    public:
        HttpRequest():_version("HTTP/1.1") {} //默认构造函数为HTTP/1.1
        void ReSet() {  //重置请求对象
            _method.clear();
            _path.clear();
            _version = "HTTP/1.1";
        ...
        void SetHeader(const std::string &key, const std::string &val) {
            _headers.insert(std::make_pair(key, val));  //std::make_pair(key, val) 创建一个键值对
        }
        //判断是否存在指定头部字段
        ...
        size_t ContentLength() const {
            // Content-Length: 1234\r\n
            bool ret = HasHeader("Content-Length");  //判断是否存在Content-Length头部字段
            if (ret == false) {
                return 0;
            }
...
// Http接收状态枚举类型，用于表示当前接收及解析的阶段状态
typedef enum {
    RECV_HTTP_ERROR,  //当前接收及解析的阶段状态为错误
    RECV_HTTP_LINE,  //当前正在接收的行数据
    RECV_HTTP_HEAD,  //当前正在接收的头部字段数据
    RECV_HTTP_BODY,  //当前正在接收的正文数据
    RECV_HTTP_OVER  //当前接收及解析的阶段状态为完成
}HttpRecvStatu;
// Http上下文类，用于存储请求和响应对象
#define MAX_LINE 8192  
class HttpContext {
    private:
        int _resp_statu; //响应状态码
        HttpRecvStatu _recv_statu; //当前的阶段状态
    ...
    private:
        bool ParseHttpLine(const std::string &line) {  //解析HTTP请求行
            ...
            std::transform(_request._method.begin(), _request._method.end(), _request._method.begin(), ::toupper);  //将method转换为大写
            ...
            for (auto &str : query_string_arry) {
                ...
                std::string val = Util::UrlDecode(str.substr(pos + 1), true);  //对值进行URL解码操作并且需要+转空格
                _request.SetParam(key, val);
            }
            return true;
        }
        bool RecvHttpLine(Buffer *buf) {  //接收并解析HTTP请求行
            ...
            bool ret = ParseHttpLine(line);  //解析HTTP请求行
            if (ret == false) {
                return false;
            }
            //首行处理完毕，进入头部获取阶段
        ...
        bool RecvHttpHead(Buffer *buf) {  //接收并解析HTTP请求头部字段
            if (_recv_statu != RECV_HTTP_HEAD) return false;  //状态不对直接返回
            //一行一行取出数据，直到遇到空行为止， 头部的格式 key: val\r\nkey: val\r\n....
            while(1){  //循环取出每一行数据
                std::string line = buf->GetLineAndPop();  //取出一行数据，包含换行符
                //2. 需要考虑的一些要素：缓冲区中的数据不足一行， 获取的一行数据超大
                if (line.size() == 0) {
                    //缓冲区中的数据不足一行，则需要判断缓冲区的可读数据长度，如果很长了都不足一行，这是有问题的
                    if (buf->ReadAbleSize() > MAX_LINE) {
        ...
        bool ParseHttpHead(std::string &line) {  //解析HTTP请求头部字段
            //key: val\r\nkey: val\r\n....
        ...
        bool RecvHttpBody(Buffer *buf) {  //接收并解析HTTP请求正文
            ...
            if (buf->ReadAbleSize() >= real_len) {
                _request._body.append(buf->Read_pos(), real_len);
                buf->MoveReadOffset(real_len);
                _recv_statu = RECV_HTTP_OVER;
                return true;
            }
            //  3.2 缓冲区中数据，无法满足当前正文的需要，数据不足，取出数据，然后等待新数据到来
            _request._body.append(buf->Read_pos(), buf->ReadAbleSize());
            buf->MoveReadOffset(buf->ReadAbleSize());  //移动读指针，将已经读取的数据从缓冲区中移除
            return true;
        }
    public:
        ...
        HttpRecvStatu RecvStatu() { return _recv_statu; }  //获取当前的阶段状态
        HttpRequest &Request() { return _request; }  //获取已经解析得到的请求信息
        //接收并解析HTTP请求
...
class HttpServer {
    ...
    private:
        ...
        void WriteReponse(const PtrConnection &conn, const HttpRequest &req, HttpResponse &rsp) {
            ...
            if (rsp._redirect_flag == true) {  //有重定向
        ...
        void FileHandler(const HttpRequest &req, HttpResponse *rsp) {
            std::string req_path = _basedir + req._path;  //将请求路径转换为实际存在的路径
            if (req._path.back() == '/')  {
                req_path += "index.html";
            }
            ...
            rsp->SetHeader("Content-Type", mime);  //设置响应头的Content-Type字段，值为文件的mime类型
            return;
        }
        //功能性请求的分类处理
        ...
        void Route(HttpRequest &req, HttpResponse *rsp) {  //路由函数，根据请求的资源路径，判断是静态资源请求，还是功能性请求
    ...
    public:
        ...
        void SetBaseDir(const std::string &path) {  //设置静态资源的根目录
